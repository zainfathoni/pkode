...
test('resolvePkode resolves to src/index.js when in the pkode package', () => {
  mockPkg({ package: { name: 'pkode' } })
  expect(require('../utils').resolvePkode()).toBe(
    require.resolve('../').replace(process.cwd(), '.')
  )
})

test('resolvePkode resolves to pkode if not in the pkode package', () => {
  mockPkg({ package: { name: 'not-pkode' } })
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolvePkode()).toBe('pkode')
})
